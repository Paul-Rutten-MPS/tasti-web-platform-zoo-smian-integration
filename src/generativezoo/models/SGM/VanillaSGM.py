...
class VanillaSGM(nn.Module):
  def __init__(self, args, in_channels=1, input_size=28):
    ...
    print("sampler_type", self.sampler_type)
    self.atol = args.atol
    self.rtol = args.rtol
    self.eps = args.eps
...
#        if not self.no_wandb:
 #         wandb.log({'samples': fig}, step = epoch)
  ...
  @torch.no_grad()
  def sample(self, num_samples=64, visualize=False):
      '''
      ...
      self.model.eval()
      logger.info(f"Sampler işlemine başlanıyor: {self.sampler_type}")
      ...
      if self.sampler_type == 'EM':
          ...
          raise ValueError(f"Geçersiz sampler tipi: {self.sampler_type}")
      
      logger.info(f"Sampler işlemi tamamlandı. Örnekler normalize ediliyor.")
      samples = samples * 0.5 + 0.5  
      samples = samples.clamp(0.0, 1.0)

      # Görselleştirme ve Base64 dönüştürme
