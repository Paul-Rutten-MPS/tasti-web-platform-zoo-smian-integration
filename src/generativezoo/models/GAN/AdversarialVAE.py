...
class AdversarialVAE(nn.Module):
    def __init__(self, input_shape, input_channels, args):
        ...
        self.vae = VanillaVAE(input_shape, input_channels, args.latent_dim, args.hidden_dims, args.lr, args.batch_size, args.kld_weight, args.loss_type).to(self.device)
    ...
    def train_model(self, data_loader, verbose = True):
        ...
        for epoch in epochs_bar:
            ...
            if acc_g_loss/len(data_loader.dataset) < best_loss:
                ...
                torch.save(self.state_dict(), os.path.join(models_dir, 'AdversarialVAE', f"AdvVAE_{self.dataset}.pt"))
                torch.save(self.discriminator.state_dict(), os.path.join(models_dir, 'AdversarialVAE', f"Discriminator_{self.dataset}.pt"))
        '''
