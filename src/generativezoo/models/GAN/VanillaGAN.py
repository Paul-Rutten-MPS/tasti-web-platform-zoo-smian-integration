...
class Generator(nn.Module):
    ...
    def sample(self, n_samples, device):
        """
        Modelden örnekler üretir ve üretilen grid görselini base64 formatında döndürür.
        
        :param n_samples: Üretilecek örnek sayısı
        :param device: Kullanılacak cihaz (CPU veya GPU)
        :return: Base64 kodlu görsel içeren liste
        """
        self.eval()
        # Rastgele latent vektörleri üret
        z = torch.randn(n_samples, self.latent_dim, 1, 1).to(device)
        imgs = self.forward(z)
        imgs = (imgs + 1) / 2  # Normalize et
        imgs = imgs.detach().cpu()

        # Görselleri ızgara halinde birleştir: sqrt(n_samples) x sqrt(n_samples)
        ...
        # Yeni bir figür ve aks oluştur, ardından grid görselini çizdir
        fig, ax = plt.subplots()
        ax.imshow(grid.permute(1, 2, 0))
        ax.axis('off')

        # Figürü belleğe kaydet ve base64 formatına çevir
